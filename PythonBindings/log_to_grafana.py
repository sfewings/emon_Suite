        ...
        try:
            for sensor in range(emonSuite.HWS_TEMPERATURES):
                p = Point("temperature").tag("sensor", f"temperature/HWS/{sensor}")\
                                        .tag("sensorGroup", "Hot water system")\
    ...
    try:
        ...
        for line in f:
            try:
                ...
                process_line(node, time, line[len(dateAndNode[0])+1: ], settings)
                lineNum = lineNum+1
...
def on_message(client, settings, msg):
    ...
    dateAndNode = line.split(',',2)
    node = dateAndNode[0].rstrip('0123456789')
    time = datetime.datetime.now()
