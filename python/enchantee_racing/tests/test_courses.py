...
def test_the_leg_table_says_which_legs_are_beats_when_the_wind_is_known():
    ...
    assert all(r["leg_type"] in ("close haul", "reach", "run") for r in rows)
    # a leg straight into a northerly is a close haul, one straight away from it is a run
    for row in rows:
        off = abs(nav.norm180(0.0 - row["bearing"]))
        expected = "close haul" if off < 40.0 else "run" if off > 140.0 else "reach"
...
def test_the_leg_type_thresholds_have_one_definition():
    ...
    assert race_module.leg_type(0.0, 10.0) == course.leg_type(0.0, 10.0) == "close haul"
