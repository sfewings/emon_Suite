...
| `depth.json` | contours, four depth bands, and the foreshore | 356 kB | 66 kB |
| `structures.json` | jetties, marinas, bridges, groynes | 216 kB | 26 kB |
| `navaids.json` | the DoT navigation aid register | 220 kB | 23 kB |

nginx gzips them, so the whole map costs 156 kB over the wire.
...
water the survey never reached. 356 kB, 326 features. This is the line that
actually changes how a leg is sailed.
...
`coverage_bbox` in the file is the survey footprint,
115.7533 to 115.8594 E, -32.0431 to -31.9589 S. Outside it there is no depth
product at all, which is a different thing from `foreshore`, and the map should not
imply coverage there. Perth Water above about 115.859 E is outside.
