...
| `depth.json` | 2 / 5 / 10 m contours and four depth bands | 277 kB | 57 kB |
| `structures.json` | jetties, marinas, bridges, lights | 219 kB | 27 kB |

nginx gzips them, so the whole map costs 125 kB over the wire.

Draw order, bottom to top: depth bands, depth contours, land, structures, then
marks and course.
...
10 m contours plus the four bands they divide the water into. 277 kB, 179 features.
...
Validation, and it is independent of both datasets: marks named SPIT sit in water
shallower than 5 m 76% of the time, against 43% for every other mark, and not one
of them reaches the `deepest` band where 16 of the others sit. The shallowest marks
of all are the Point Walter boat-ramp piles and Waratah Spit. If a regeneration
ever breaks that relationship, distrust it before you trust it.
...
Done, in `config/structures.json`, generated by `scripts/gen_structures.py`. 219 kB,
1046 features. OSM again, same extent and licence as `coast.json`, so the two line
...
| `light` | 24 | `man_made=lighthouse` plus seamark lights and beacons | `#c8402c` |
...
fixed structure), `wreck`, and `berth`.
