...
static/                 app.js and app.css, the race screen
                        ...
                        palette.js, the speed colour scale, GENERATED by
...
- Config lives in readable JSON on disk, one copy, read by both the server and the
  browser.
- The **day's trail is server state too**, and in memory only (DESIGN 12.6). Its day
  boundary is an explicit +8 offset in `store.py`, never the process timezone: the
  Dockerfile sets `TZ=UTC`, so `localtime()` would roll the trail over at 08:00 Perth,
  mid-sail. It is served by `/api/trail` with a cursor, not on `/api/state`, which stays
  one small document for all three pages.
