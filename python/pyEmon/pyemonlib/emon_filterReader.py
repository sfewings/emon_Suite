...
class emon_filterReader:
    ...
    def printException(self, exceptionSource, reading, ex):
        if( self.lineNumber == -1):
            print(f"EmonReader:{exceptionSource} {reading} - {ex}")
        else:
            print(f"EmonReader:{exceptionSource}:{self.lineNumber}, {reading} - {ex}")
    ...
    def rainMessage(self,  reading, nodeSettings, time ):
    ...
    def otherMessage(self, reading, nodeSettings, time ):
        print(reading)


    def process_line(self, command, line , time=None ):
        if(command in self.dispatch.keys()):
            self.dispatch[command](line, self.settings[command], time)